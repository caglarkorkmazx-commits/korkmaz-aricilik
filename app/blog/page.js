  ...
  return (
    <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 20px', minHeight: '80vh' }}>
      <div style={{ marginBottom: '40px' }}>
        ...
        <p style={{ color: '#888' }}>Belfast ana arı bakımı, koloni yönetimi ve doğal bal üretimi üzerine teknik makaleler.</p>
      </div>

      {loading ? (
        <div style={{ color: '#aaa' }}>Yazılar yükleniyor...</div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '30px' }}>
          {blogs.map((blog) => (
            <article key={blog.id} style={{ backgroundColor: '#1a1a1a', border: '1px solid #333', borderRadius: '12px', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
              ...
              <div style={{ padding: '20px', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                ...
                <Link 
                  href={`/blog/${blog.id}`} 
                  style={{ color: '#f59e0b', fontSize: '14px', fontWeight: 'bold', textDecoration: 'none' }}
                >
                  Devamını Oku &rarr;
                </Link>
              </div>
